...
export default function Outcomes() {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {/* MIDDLE SECTION: 2 CHARTS SIDE BY SIDE (Exact Screenshot Match) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Left Chart: Mood Trend */}
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">Mood Trend</h3>
                <p className="text-xs text-slate-500 font-medium">Self-reported mood over time for {activeClient.name}</p>
              </div>
            </div>
            <Badge className="bg-purple-50 text-[#5e2be2] border-purple-200 font-bold text-[10px]">
              {activeClient.therapyType.split("·")[0].trim()}
            </Badge>
          </div>

          <div className="h-64 w-full pt-2">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={activeClient.moodTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <defs>
                ...
                <XAxis dataKey="session" axisLine={false} tickLine={false} tick={{ fontSize: 11, fill: '#64748b' }} dy={10} />
                ...
                />
                <Area type="monotone" dataKey="score" stroke="#5e2be2" strokeWidth={3} fillOpacity={1} fill="url(#purpleGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
